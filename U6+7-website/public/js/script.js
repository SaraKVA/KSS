...
var lux_m = [];
var count = [];*/
function OrientationValues_m() {return {
	alpha_m: [],
	beta_m: [],
	gamma_m: [],
	alphaG_m: [],
...
//orientation_buf_m= new OrientationValues_m();
//feature_buf = new Features();
...
function onChange(){
	...
	//document.getElementById("debug").innerHTML = "onChnage" + orientation_buf_m;
	data_count = 0;
    ...
    if(checkbox.checked){
		...
		document.getElementById("debug").innerHTML = "Classifying";
		...
		if (window.window.DeviceMotionEvent) {
			...
			interval = window.setInterval(classifyData, 1000 / UPLOAD_RATE);
		} else {
        ...
        window.clearInterval(interval);
        writePoint();
        document.getElementById("debug").innerHTML = "Not classifying."
    }
}
...
function classifyData(){
	//data_count = 0;
	//document.getElementById("debug").innerHTML = "inside classifier" + ": " + data_count;
	...
	data_count++;
	orientation_buf_m.count.push(data_count);
	orientation_buf_m.alpha_m.push(milli_mean_int(v.alpha)); //use mean * 1000 (int for efficiency) (bei schicken jeder 1ms => rauschen(min, max windowing später))
	orientation_buf_m.beta_m.push(milli_mean_int(v.beta));
	orientation_buf_m.gamma_m.push(milli_mean_int(v.gamma));
	orientation_buf_m.alphaG_m.push(milli_mean_int(v.alphaG));
	orientation_buf_m.betaG_m.push(milli_mean_int(v.betaG));
	orientation_buf_m.gammaG_m.push(milli_mean_int(v.gammaG));
	orientation_buf_m.lux_m.push(mean_lux(v.lux));

	if(orientation_buf_m.alphaG_m.length > 19){
		var all_means = [];
		var temp_O = orientation_buf_m;
		orientation_buf_m = new OrientationValues_m();
		all_means.push(temp_O.alpha_m);
		all_means.push(temp_O.alphaG_m);
		all_means.push(temp_O.beta_m);
		all_means.push(temp_O.betaG_m);
		all_means.push(temp_O.count);
		all_means.push(temp_O.gamma_m);
		all_means.push(temp_O.gammaG_m);
		all_means.push(temp_O.lux_m);
		var features = [];
		//document.getElementById("debug").innerHTML = "inside classifier" + ": " + all_means[0].length ;

		features = featureExtraction(all_means);
		var prediction = clf.predict(features);
		switch(prediction){
			case 0:
				prediction_str = "sitting";
				console.log(prediction);
				document.body.style.backgroundColor = "gray";
				document.getElementById("predict").textContent=prediction_str
				document.getElementById("predict").style.fontSize = "x-small";
				break;
			case 1:
				prediction_str = "standing";
				console.log(prediction);
				document.body.style.backgroundColor = "powderblue";
				document.getElementById("predict").textContent=prediction_str;
				document.getElementById("predict").style.fontSize = "medium";
				break;
			case 2:
				prediction_str = "walking";
				console.log(prediction);
				document.body.style.backgroundColor = "powderblue";
				document.getElementById("predict").textContent=prediction_str;
				document.getElementById("predict").style.fontSize = "xx-large";
				break;
		}
		//all_means = [];
		//features = [];
...
/*
	...
	//document.getElementById("predict").textContent="FEATURE EXTRACT___" + features.length;
